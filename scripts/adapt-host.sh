...
issue_cert_webroot() {
  ...
  log "正在为 ${site} 申请 Let's Encrypt 证书..."
  certbot certonly --webroot -w "$ACME_WEBROOT" \
    -d "$site" \
    -m "$email" \
    --agree-tos --no-eff-email --non-interactive
  [ -f "$live/fullchain.pem" ] || fail "证书申请失败：$live 不存在"
  printf '%s' "$live"
}
...
integrate_nginx() {
  ...
  nginx_http_only_server "$site" > "$dest"
  reload_nginx
  ...
  live="$(issue_cert_webroot "$site" "$email")"
  nginx_ssl_server "$site" "$live/fullchain.pem" "$live/privkey.pem" > "$dest"
  reload_nginx
...
integrate() {
  ...
  case "$MODE" in
    ...
    behind-unknown)
      ...
      log ""
      log "Nginx:"
      nginx_http_only_server "${SITE_ADDRESS}"
      log ""
      log "Caddy:"
      caddy_site_block "${SITE_ADDRESS}"
...
case "$PHASE" in
  ...
  all) all ;;
  *)
    echo "用法: $0 detect|configure|integrate|all"
    exit 1
    ;;
esac
